const STORAGE_KEY = 'aiva_dashboard_v11_7_revenue_section_site';
...
function revenueRows(){
  return (state.revenueRows||[]).filter(r=>{
    const total=datesInRange().reduce((a,d)=>a+Number(r.dates?.[d]||0),0);
    return true;
  });
}
function revenueTotal(){
  return revenueRows().reduce((sum,row)=>sum+datesInRange().reduce((a,d)=>a+Number(row.dates?.[d]||0),0),0);
}
function revenueSummary(){
  const total=revenueTotal();
  return revenueRows().map(row=>{
    const amount=datesInRange().reduce((a,d)=>a+Number(row.dates?.[d]||0),0);
...
function renderRevenue(){
  ...
  const total = revenueTotal();
  document.getElementById('revenueKpis').innerHTML = kpis([
  ...
  document.getElementById('revenueMatrix').innerHTML = head + body + foot + '</tbody></table>';
  document.querySelector('#revenueSummaryTable tbody').innerHTML = revenueSummary().map(r=>`<tr><td>${r.name}</td><td>${money(r.amount)}</td><td>${r.share}%</td></tr>`).join('');
}
...
function renderAll(){
  renderControls();
  renderRnp(); renderHome(); renderPlans(); renderMarketing(); renderSales(); renderClinic(); renderDoctors(); renderRevenue(); renderFinance(); renderKnowledge(); renderUsers();
}
